...
{
	return sqrt_(u.x*u.x + u.y*u.y + u.z*u.z + w*w);
}
...
Quaternion slerp::RotateAboutAxis(Point3D pt, double angle, Point3D axis)
{
	...
	q.w = cos_(0.5*angle);
	q.u.x = sin_(0.5*angle)*axis.x;
	q.u.y = sin_(0.5*angle)*axis.y;
	q.u.z = sin_(0.5*angle)*axis.z;
...
void slerp::Slerp1(Quaternion v0, Quaternion v1, Quaternion &qr, double t) {
	...
	double theta_0 = acos_(dot);        // theta_0 = angle between input vectors
	double theta = theta_0*t;          // theta = angle between v0 and result
	double sin_theta = sin_(theta);     // compute this value only once
	double sin_theta_0 = sin_(theta_0); // compute this value only once

	double s0 = cos_(theta) - dot * sin_theta / sin_theta_0;  // == sin(theta_0 - theta) / sin(theta_0)
